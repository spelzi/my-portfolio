...
vi.mock("../../src/Component/Admin/AdminStore", () => ({
  AdminStore: {
    getVideos: vi.fn((defaults) => Promise.resolve(defaults)),
  },
}));
...
describe("BlogVideo", () => {
  ...
  it("renders all default video titles", () => {
    render(<BlogVideo />);
    expect(screen.getByText("Smart Money Concepts — Full Breakdown")).toBeInTheDocument();
    expect(screen.getByText("How to Build a React App from Scratch")).toBeInTheDocument();
  });
  ...
  it("renders the 'More on YouTube' channel link", () => {
    ...
    expect(link.closest("a")).toHaveAttribute("href", "https://www.youtube.com/@St_manuel1");
  });
  ...
  describe("when a video has a youtubeId", () => {
    ...
    it("renders a Watch on YouTube link with the correct URL", async () => {
      const { AdminStore } = await import("../../src/Component/Admin/AdminStore");
      AdminStore.getVideos.mockResolvedValueOnce(videosWithId);

      render(<BlogVideo />);
      const watchLink = await screen.findByText(/Watch on YouTube →/i);
    ...
    it("mounts the preview iframe on hover", async () => {
      ...
      render(<BlogVideo />);
      const titleEl = await screen.findByText("Test Video");
      const card = titleEl.closest(".video-card");
